/**
 * app-views.js — View rendering and UI orchestration functions
 *
 * Plain <script> (not ES module). Loaded BEFORE main.js so functions
 * are available globally when main.js module runs.
 *
 * This file contains all view/render/UI functions that are NOT yet
 * extracted into ES modules under modules/. It relies on window.*
 * globals set up by state proxy in main.js (currentFilters, allDeals,
 * STAGE_CONFIG, etc.) and on functions exposed by the existing modules.
 *
 * Functions defined here are automatically available as globals.
 * main.js wires them via typeof window.X === 'function' checks.
 */

/* jshint esversion: 11 */

// Unit summary stage order constant (not exported from state.js)
