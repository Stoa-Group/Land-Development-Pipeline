...
- [ ] Accept JSON body with `email` (required), `name` (optional), `userId` (optional).
- [ ] Resolve user by email (look up `auth.[User]` by Email).
- [ ] Return `{ success: true, data: { token: "<JWT>", user: { userId, username, email, fullName } } }` in the same format as login.
