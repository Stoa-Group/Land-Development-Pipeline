...
- **ASANA_ACCESS_TOKEN** or **ASANA_PAT:** Required for Asana API calls.
- **ASANA_WORKSPACE_GID** (optional): Default workspace when the client does not send `workspace`.
- **ASANA_START_DATE_CUSTOM_FIELD_GID** (optional): GID of the Asana custom field **"Start Date"** (type: date). When set, the remedy endpoint updates this custom field instead of the task’s Due Date. Example (Deal Pipeline project): `1207906700131717`.
...
1. **Override Asana date with database date** (and **Fill start date in Asana with database date** when Asana has no start date)  
   - The frontend calls: `API.updateAsanaTaskStartDate(taskGid, dateStr)` where `dateStr` is `YYYY-MM-DD` (the database start date).  
   - Backend: Update the Asana task’s **custom field "Start Date"** to that date. Do **not** change the task’s **Due Date** (`due_on`). Start Date is a custom field in Asana; the app only syncs that field here. When `ASANA_START_DATE_CUSTOM_FIELD_GID` is set, the backend should PATCH the task’s custom_fields with that GID and the date; when not set, the backend may fall back to updating `due_on` for backward compatibility, but the intended behavior is to set the Start Date custom field.
